...
import { supabase, getCurrentUser, auth, orders as ordersAPI, favorites } from '@/lib/supabase';
...
export default function ProfilePage({ params }: ProfilePageProps) {
  ...
  useEffect(() => {
    const checkUser = async () => {
      try {
        ...
        // إحضار البيانات بناءً على التبويب النشط
        if (activeTab === 'orders') {
    ...
    checkUser();
  }, [params.locale, router, activeTab]);
  ...
  const fetchFavorites = async (userId: string) => {
    try {
      const { data } = await favorites.getUserFavorites(userId);
      if (data) {
        setFavoriteProducts(data.map((item: any) => item.products));
      }
    } catch (error) {
                        ...
                        <div key={product.id} className="border rounded-lg overflow-hidden group">
                          <div className="relative h-48">
                            ...
                            <button 
                              ...
                              className="absolute top-2 right-2 bg-white rounded-full p-1.5 text-red-500 shadow-sm hover:bg-red-50"
                          ...
                          <div className="p-4">
                            ...
                            <div className="flex justify-between items-center mt-3">
                              <span className="font-semibold">{product.price.toFixed(2)} {t('common.price')}</span>
                              <Link
                                href={`/${params.locale}/product/${product.id}`}
                                className="text-helden-purple hover:text-helden-purple-dark font-medium text-sm"
                              >
